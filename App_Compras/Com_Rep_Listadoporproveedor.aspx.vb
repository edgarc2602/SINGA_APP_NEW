...
Partial Class App_Compras_Com_Rep_Listadoporproveedor
    ...
    <Web.Services.WebMethod()>
    Public Shared Function prefijo() As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""

        sqlbr.Append("select distinct prefijo from tb_cliente_inmueble where id_cliente = 130 and id_status = 1 and materiales = 0")
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        sql = "["
        If dt.Rows.Count > 0 Then
            For x As Integer = 0 To dt.Rows.Count - 1
                If x > 0 Then sql += ","
                sql += "{id:'" & dt.Rows(x)("prefijo") & "'," & vbCrLf
                sql += "desc:'" & dt.Rows(x)("prefijo") & "'}" & vbCrLf
            Next
        End If
        sql += "]"
        Return sql
