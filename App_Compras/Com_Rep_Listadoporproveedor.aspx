<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Com_Rep_Listadoporproveedor.aspx.vb" Inherits="App_Compras_Com_Rep_Listadoporproveedor" %>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#btimprime').click(function () {
                var formula = '{tb_listadomaterial.id_status} <> 5 and {tb_listadomaterial.anio}=' + $('#txanio').val() + ' and {tb_listadomaterial.mes}=' + $('#dlmes').val()
                if ($('#dlcliente').val() != 0) {
                    formula += ' and {tb_listadomaterial.id_cliente}=' + $('#dlcliente').val()
                } 
                if ($('#dlproveedor').val() != 0) {
                    formula += ' and {tb_proveedorinmueble.id_proveedor} =' + $('#dlproveedor').val()
                }
                if ($('#dlestado').val() != 0) {
                    formula += ' and {tb_cliente_inmueble.id_estado}=' + $('#dlestado').val()
                }
                if ($('#dlprefijo').val() != 0) {
                    formula += ' and {tb_cliente_inmueble.prefijo} = "' + $('#dlprefijo').val() +'"'
                }
                //alert(formula);
                if ($('#dlcliente').val() == 130) {
                    if ($('#dlprefijo').val() == 'DAZ' || $('#dlprefijo').val() == 'PP') {
                    ...
                    if ($('#dlcliente').val() == 2223) {
                        window.open('../RptForAll.aspx?v_nomRpt=listadoentregaoline.rpt&v_formula=' + formula, '', 'width=850, height=600, left=80, top=120, resizable=no, scrollbars=no');
                    } else {
        ...
        function cargaprefijo() {
            PageMethods.prefijo(function (opcion) {
                var opt = eval('(' + opcion + ')');
                var lista = '';
                for (var list = 0; list < opt.length; list++) {
                    lista += '<option value="' + opt[list].id + '">' + opt[list].desc + '</option>';
                };
                $('#dlprefijo').append(inicial);
                $('#dlprefijo').append(lista);
            }, iferror);
        }
        ...
        <div class="wrapper">
            ...
            <div class="content-wrapper">
                <div class="content-header">
                    <h1>Listados de entrega por proveedor<small>Compras</small></h1>
                    <ol class="breadcrumb">
                        <li><a href="../Home.aspx"><i class="fa fa-dashboard"></i>Home</a></li>
                        <li><a>Compras</a></li>
                        <li class="active">Listados de entrega</li>
                    </ol>
                </div>
                <div class="content">
                    <div class="row" id="dvdatos">
                        <div class="box box-info">
                            ...
                            <div class="row">
                                <div class="col-lg-2 text-right">
                                    <label for="dlprefijo">Prefijo:</label>
                                </div>
                                <div class="col-lg-2">
                                    <select id="dlprefijo" class="form-control"></select>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-lg-2 text-right">
