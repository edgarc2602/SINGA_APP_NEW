...
Imports System.Xml
Partial Class App_Compras_Com_Cat_productoprecio1
    ...
    <Web.Services.WebMethod()>
    Public Shared Function materiales(ByVal clave As String) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        sqlbr.Append("select a.clave as 'td','', '' as 'td','', b.descripcion as 'td','', c.descripcion as 'td','', (select 'form-control text-right tbeditar' as '@class', cast(a.precio as numeric(12,2)) as '@value' for xml path('input'),root('td'),type),'', " & vbCrLf)
        ...
        sqlbr.Append("from tb_productoprecio a inner join tb_producto b on a.clave = b.clave " & vbCrLf)
        sqlbr.Append("inner join tb_unidadmedida c on b.id_unidad = c.id_unidad " & vbCrLf)
        sqlbr.Append("where a.id_proveedor = " & clave & " order by b.descripcion for xml path('tr'), root('tbody')")
