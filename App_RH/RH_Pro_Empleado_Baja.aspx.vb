...
Partial Class App_RH_RH_Pro_Empleado_Baja
    ...
    Public Shared Function empleados(ByVal pagina As Integer, ByVal cliente As Integer, ByVal inmueble As Integer, ByVal noemp As Integer, ByVal nombre As String) As String
        ...
        sqlbr.Append("puesto as 'td','', turno as 'td','',id_puesto as 'td','', id_turno as 'td','', estatus as 'td','', id_plantilla as 'td','', posicion as 'td'  from (" & vbCrLf)
        ...
        sqlbr.Append("a.id_plantilla, e.posicion" & vbCrLf)
    ...
    Public Shared Function guarda(ByVal registro As String, ByVal fecha As String, ByVal cliente As String, ByVal puesto As String, ByVal sucursal As String, ByVal ubicacion As String, ByVal horario As String, ByVal fbaja As String, ByVal empleado As String, ByVal idcli As Integer, ByVal vacante As Integer, ByVal motivo As String) As String
        ...
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""
        sqlbr.Append("select rfc, curp, ss, paterno + ' ' + rtrim(materno) + ' ' + nombre as empleado, tel1 from tb_empleado where id_empleado = " & empleado & "" & vbCrLf)
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
        ...
        generacorreo.correobaja(fecha, "Baja de Empleado", cliente, puesto, sucursal, fbaja, nombre, rfc, curp, ss, idcli, motivo, tel1)
    ...
    Public Shared Function coordina(ByVal cliente As Integer) As String
        ...
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""
        sqlbr.Append("select id_coordinadorrh, b.nombre  + ' ' +  b.paterno+ ' ' + rtrim(b.materno) as nombre " & vbCrLf)
        sqlbr.Append("from tb_cliente a inner join tb_empleado b on a.id_coordinadorrh = b.id_empleado " & vbCrLf)
        sqlbr.Append("where a.id_cliente = " & cliente & "")
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
            sql = "{id: '" & dt.Rows(0)("id_coordinadorrh") & "', nombre:'" & dt.Rows(0)("nombre") & "'}"
        End If
        Return sql
    End Function
