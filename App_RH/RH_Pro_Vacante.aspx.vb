...
Partial Class App_RH_RH_Pro_Vacante
    ...
    Public Shared Function empleado(ByVal cliente As Integer) As String
        ...
        sqlbr.Append(" d.nombre + ' ' + rtrim(d.paterno) + ' ' + rtrim(d.materno)  as coordina, a.id_coordinadorrh" & vbCrLf)
        ...
        sqlbr.Append("inner join tb_empleado d on a.id_coordinadorrh = d.id_empleado " & vbCrLf)
        sqlbr.Append("where a.id_cliente = " & cliente & "")
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
            sql += "{nombre: '" & dt.Rows(0)("empleado") & "', coordina: '" & dt.Rows(0)("coordina") & "', idcoordina: '" & dt.Rows(0)("id_coordinadorrh") & "'}" & vbCrLf
        End If
        Return sql
    End Function
    ...
    <Web.Services.WebMethod()>
    Public Shared Function estructura(ByVal inmueble As Integer, ByVal puesto As Integer, ByVal turno As Integer) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        ...
        sqlbr.Append("case when b.formapago = 1 then 'Quincenal' else 'Semanal' end as 'td','', case when tiponomina = 1 then 'Normal' else 'Jornal' end as 'td',''," & vbCrLf)
        sqlbr.Append("case when sexo =1 then 'Masculino' when sexo = 2 then 'Femenino' when sexo = 3 then 'Indistinto' end as 'td','', " & vbCrLf)
        sqlbr.Append(" cast(smntope as numeric(12,2)) as 'td','',")
        ...
        sqlbr.Append("where b.id_inmueble = " & inmueble & " and b.id_status = 1 " & vbCrLf)

        If puesto <> 0 Then sqlbr.Append(" and b.id_puesto = " & puesto & "" & vbCrLf)
        If turno <> 0 Then sqlbr.Append(" and b.id_turno = " & turno & "" & vbCrLf)
        sqlbr.Append("order by b.id_plantilla, a.posicion for xml path('tr'), root('tbody')")
