...
Partial Class App_RH_RH_Con_Asistencia
    ...
    Public Shared Function asistencia(ByVal pagina As Integer, ByVal cliente As Integer, ByVal inmueble As Integer, ByVal fecini As String, ByVal fecfin As String, ByVal forma As Integer, ByVal turno As Integer) As String
        ...
        sqlbr.Append("(select 'bg-navy' as '@class', d7 as 'span' FOR XML path('th'), type) from tb_periodonomina where finicio = '" & Format(vfecini, "yyyyMMdd") & "' and ffin = '" & Format(vfecfin, "yyyyMMdd") & "' for xml path('tr'), root('thead'),type)," & vbCrLf)
        sqlbr.Append("(select cliente as 'td','', inmueble as 'td','', id_empleado as 'td','', empleado as 'td','', isnull(a,'') as 'td','',isnull(b,'') as 'td',''," & vbCrLf)
        sqlbr.Append("isnull(c,'') as 'td','',isnull(d,'') as 'td','',isnull(e,'') as 'td','',isnull(f,'') as 'td','', isnull(g,'') as 'td' " & vbCrLf)
        sqlbr.Append("from (" & vbCrLf)
    ...
    <Web.Services.WebMethod()>
    Public Shared Function empleado() As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""

        sqlbr.Append("select id_empleado, paterno + ' '+ rtrim(materno) + ' ' + nombre as empleado from tb_empleado where id_status = 2 and esencargado = 1 order by empleado")
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        sql = "["
