            ...
            PageMethods.vacantes($('#dlcliente').val(), $('#dlinmueble').val(), function (res) {
                if (ren != '') {
                    ...
                    $('#tblista table tbody tr').on('click', '.tbrechaza', function () {
                        var r = confirm("Si presiona el botón ACEPTAR, se cancela la activación de este candidato y se reactiva la vacante, al presionar CANCELAR, el candidato queda eliminado del proceso pero la vacante no se reactiva");
                        if (r == true) {
                            //alert($('#idusuario').val())
                            var f = new Date();
                            var mm = f.getMonth() + 1
