<%@ Page Language="VB" AutoEventWireup="false" CodeFile="RH_Cat_Turno.aspx.vb" Inherits="App_RH_RH_Cat_Turno" %>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#btagrega').click(function () {
                if (valida()){
                    PageMethods.guarda($('#idturno').val(), $('#txdesc').val(), function () {
                        cargalista();
                        $('#idturno').val(0);
                        $('#txdesc').val('');
                    }, iferror);
        ...
        function valida() {
            if ($('#txdesc').val() == '') {
                alert('Debe capturar una descripción para el turno');
                return false;
            }
            for (var x = 0; x < $('#tblista tbody tr').length; x++) {
                if ($('#tblista tbody tr').eq(x).find('td').eq(1).text() == $('#txdesc').val() && $('#idturno').val() == 0) {
                    alert('La descripción de turno capturada ya esta registrada no puede duplicar');
                    return false;
                }
            }
            return true;
        ...
        function cuentaturno() {
            PageMethods.contarturno(function (cont) {
        ...
        function cargalista() {
            PageMethods.turno($('#hdpagina').val(), function (res) {
                var ren = $.parseHTML(res);
                $('#tblista tbody').remove();
                $('#tblista').append(ren);
                $('#tblista tbody tr').on('click', '.tbeditar', function () {
                    $('#idturno').val($(this).closest('tr').find('td').eq(0).text());
...
<body class="skin-blue sidebar-mini">
    <form id="form1" runat="server">
        ...
        <asp:HiddenField ID="idturno" runat="server" value="0"/>
        <div class="wrapper">
