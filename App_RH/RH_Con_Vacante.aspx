...
;           PageMethods.permiso($('#idusuario').val(), function (detalle) {
            ...
            $('#btexporta').click(function () {
                ...
                window.open('RH_Descarga_vacante.aspx?cliente=' + $('#dlcliente').val() + '&fecini=' + $('#txfecini').val() + ' &fecfin=' + $('#txfecfin').val() + ' &estatus=' + $('#dlestatus').val() + ' &sucursal=' + $('#dlsucursal').val() + '&gerente=' + $('#dlgerente').val(), + '&folio=' + $('#txfolio').val() + '&estado=' + $('#dlestado').val(), '_blank');
            })
        ...
        function cuentavacante() {
            ...
            PageMethods.contarvacante($('#dlcliente').val(), $('#txfecini').val(), $('#txfecfin').val(), $('#dlestatus').val(), $('#dlsucursal').val(), $('#dlgerente').val(), $('#txfolio').val(), $('#dlestado').val(),  function (cont) {
        ...
        function cuentatotal() {
            PageMethods.contartotal($('#dlcliente').val(), $('#txfecini').val(), $('#txfecfin').val(), $('#dlestatus').val(), $('#dlsucursal').val(), $('#dlgerente').val(), $('#txfolio').val(), $('#dlestado').val(),  function (cont) {
                var opt = eval('(' + cont + ')');
        ...
        function cargaestado(idcte) {
            PageMethods.estados(idcte, function (opcion) {
                ...
                $('#dlestado').change(function () {
                    cargainmueble($('#dlcliente').val(), $('#dlestado').val());
                })
            }, iferror);
        }
        function cargainmueble(idcte, idedo) {
            PageMethods.inmueble(idcte, idedo, function (opcion) {
                var opt = eval('(' + opcion + ')');
                var lista = '';
                for (var list = 0; list < opt.length; list++) {
                    lista += '<option value="' + opt[list].id + '">' + opt[list].desc + '</option>';
        ...
        function cargalista() {
            waitingDialog({});
            PageMethods.vacantes($('#dlcliente').val(), $('#txfecini').val(), $('#txfecfin').val(), $('#dlestatus').val(), $('#revisa').val(), $('#idpuesto').val(), $('#hdpagina').val(), $('#dlsucursal').val(), $('#dlgerente').val(), $('#idusuario').val(), $('#txfolio').val(), $('#dlestado').val(), function (res) {
                closeWaitingDialog();
                var ren = $.parseHTML(res);
                if (ren == null) {
                    ...
                    $('#tblista table tbody tr').on('click', '.tbeditar', function () {
                        switch ($(this).closest('tr').find('td').eq(14).text()) {
                    ...
                    $('#tblista table tbody tr').on('click', '.tbliberar', function () {
                        switch ($(this).closest('tr').find('td').eq(14).text()) {
                            ...
                            case 'Cancelada':
                                ...
                                break;
                            default:
                                if ($(this).closest('tr').find('td').eq(15).text() == '') {
                    ...
                    <div class="row" id="tblista">
                        <div class="box box-info">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-1 text-right">
                                    <label for="dlsucursal">Sucursal:</label>
                                </div>
                                <div class="col-lg-3">
                                ...
                                <table class="table table-condensed h6">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="bg-light-blue-gradient"><span>Estado</span></th>
