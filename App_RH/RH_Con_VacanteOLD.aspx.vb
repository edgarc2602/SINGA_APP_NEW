...
Partial Class App_RH_RH_Con_Vacante
    ...
    <Web.Services.WebMethod()>
    Public Shared Function vacantes(ByVal cliente As Integer, ByVal fini As String, ByVal ffin As String, ByVal estatus As Integer, ByVal revisa As Integer, ByVal puesto As Integer, ByVal pagina As Integer, ByVal inmueble As Integer, ByVal gerente As Integer) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        ...
        Else
            ...
            Else
                If puesto = 40 Then
                    ...
                    sqlbr.Append("'' as 'td','', '' as 'td','',") ' ESTO ES PARA CONSULTA GENERAL Y SOLO PODER ASIGNAR RECLUTADOR
                Else
                    sqlbr.Append("'' as 'td','', '' as 'td','','' as 'td','', '' as 'td','',") ' ESTO ES PARA CONSULTA GENERAL SIN PODER EDITAR
                End If
            End If
        End If
        sqlbr.Append("id_vacante as 'td','', cliente as 'td','', sucursal as 'td','', gerente as 'td','', fechaalta as 'td','',tipo as 'td','', " & vbCrLf)
        ...
        sqlbr.Append("left outer join tb_turno f on a.id_turno = f.id_turno left outer join tb_puesto k on a.id_puesto = k.id_puesto " & vbCrLf)
    ...
    <Web.Services.WebMethod()>
    Public Shared Function contarvacante(ByVal cliente As Integer, ByVal fini As String, ByVal ffin As String, ByVal estatus As Integer, ByVal inmueble As Integer, ByVal gerente As Integer) As String
        ...
        sqlbr.Append("SELECT COUNT(*)/50 + 1 as Filas, COUNT(*) % 50 as Residuos FROM tb_vacante a inner join tb_cliente b on a.id_cliente = b.id_cliente where a.id_status = " & estatus & "" & vbCrLf)
    ...
    <Web.Services.WebMethod()>
    Public Shared Function contartotal(ByVal cliente As Integer, ByVal fini As String, ByVal ffin As String, ByVal estatus As Integer, ByVal inmueble As Integer, ByVal gerente As Integer) As String
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""
        ...
        sqlbr.Append("SELECT count(id_vacante) as total FROM tb_vacante a inner join tb_cliente b on a.id_cliente = b.id_cliente where a.id_status = " & estatus & "" & vbCrLf)
        If cliente <> 0 Then sqlbr.Append("and a.id_cliente = " & cliente & "")
        If inmueble <> 0 Then sqlbr.Append("and a.id_inmueble  = " & inmueble & "" & vbCrLf)
        If gerente <> 0 Then sqlbr.Append("and b.id_operativo  = " & gerente & "" & vbCrLf)
        If fini <> "" Then sqlbr.Append("and cast(fechaalta as date) between '" & Format(vfecini, "yyyyMMdd") & "' and '" & Format(vfecfin, "yyyyMMdd") & "'")
