...
Partial Class App_RH_RH_Cat_Empleado
    ...
    Public Shared Function empresa(ByVal cliente As Integer) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""

        sqlbr.Append("select a.id_empresa, b.nombre from tb_cliente_empresa a inner join tb_empresa b on a.id_empresa = b.id_empresa where a.id_cliente =" & cliente & " order by nombre")
    ...
    Public Shared Function cargavacante(ByVal vacante As Integer) As String
        ...
        sqlbr.Append("select a.id_cliente, a.id_inmueble, b.id_puesto, b.id_turno, a.id_plantilla, b.smntope, b.jornal , b.formapago, isnull(c.id_empresa,0) as id_empresa, a.posicion " & vbCrLf)
        sqlbr.Append("from tb_vacante a inner join tb_cliente_plantilla b on a.id_plantilla = b.id_plantilla" & vbCrLf)
        sqlbr.Append("left outer join tb_cliente_empresa c on a.id_cliente = c.id_cliente ")
        ...
        If dt.Rows.Count > 0 Then
            ...
            sql += "formapago:'" & dt.Rows(0)("formapago") & "', empresa:'" & dt.Rows(0)("id_empresa") & "', posicion:'" & dt.Rows(0)("posicion") & "'}"
        End If
        Return sql
    End Function
    ...
    <Web.Services.WebMethod()>
    Public Shared Function lugar() As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""

        sqlbr.Append("select abreviatura, descripcion from tb_estado order by descripcion ")
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        sql = "["
        If dt.Rows.Count > 0 Then
            For x As Integer = 0 To dt.Rows.Count - 1
                If x > 0 Then sql += ","
                sql += "{id:'" & dt.Rows(x)("abreviatura") & "'," & vbCrLf
                sql += "desc:'" & dt.Rows(x)("descripcion") & "'}" & vbCrLf
            Next
        End If
        sql += "]"
    ...
    Public Shared Function documento() As String
        ...
        sqlbr.Append("select id_documentovacante, nombre_documento from tb_documentosvacante where id_status = 1 order by nombre_documento")
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        sql = "["
        If dt.Rows.Count > 0 Then
            For x As Integer = 0 To dt.Rows.Count - 1
                If x > 0 Then sql += ","
                sql += "{id:'" & dt.Rows(x)("id_documentovacante") & "'," & vbCrLf
                sql += "desc:'" & dt.Rows(x)("nombre_documento") & "'}" & vbCrLf
            Next
        End If
        sql += "]"
        Return sql
    ...
    <Web.Services.WebMethod()>
    Public Shared Function actualizadocto(ByVal empleado As Integer, ByVal tipo As Integer, ByVal docto As String) As String

        Dim sql As String = "insert into tb_documentosvacante_empleado (id_empleado,id_documentovacante,archivo, fecha) values (" & empleado & "," & tipo & ",'" & docto & "', getdate());"
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim mycommand As New SqlCommand(sql, myConnection)
        myConnection.Open()
        mycommand.ExecuteNonQuery()
    ...
    Public Shared Function listadocto(ByVal emp As Integer) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        sqlbr.Append("select a.id_documentovacante as 'td','', b.nombre_documento as 'td','', archivo as 'td','', " & vbCrLf)
        ...
        sqlbr.Append("From tb_documentosvacante_empleado a inner join tb_documentosvacante b on a.id_documentovacante = b.id_documentovacante " & vbCrLf)
        sqlbr.Append("where id_empleado = " & emp & "  for xml path('tr'), root('tbody')")
        Dim mycommand As New SqlCommand(sqlbr.ToString(), myConnection)
        myConnection.Open()
        Dim xdoc1 As New XmlDocument()
        Dim xrdr1 As XmlReader
    ...
    <Web.Services.WebMethod()>
    Public Shared Function eliminaa(ByVal folio As Integer, ByVal docto As Integer) As String

        Dim sql As String = "delete from tb_documentosvacante_empleado where id_empleado=" & folio & " and id_documentovacante = " & docto & ";"
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim mycommand As New SqlCommand(sql, myConnection)
        myConnection.Open()
        mycommand.ExecuteNonQuery()
