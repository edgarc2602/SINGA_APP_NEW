...
Partial Class App_Operaciones_Ope_Pro_listadogeneral
    ...
    <Web.Services.WebMethod()>
    Public Shared Function contarlistados(ByVal cliente As Integer, ByVal mes As Integer, ByVal anio As Integer, ByVal estado As Integer, ByVal tipo As Integer) As String
    ...
    Public Shared Function guarda(ByVal registro As String) As String
        ...
        Try
            ...
            Dim mycommand As New SqlCommand("sp_listadomaterialg", myConnection, trans)
            mycommand.CommandType = CommandType.StoredProcedure
            mycommand.Parameters.AddWithValue("@Cabecero", registro)
            mycommand.ExecuteNonQuery()
    ...
    <Web.Services.WebMethod()>
    Public Shared Function validalistados(ByVal cliente As Integer, ByVal tipo As Integer, ByVal mes As Integer, ByVal anio As Integer) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""

        sqlbr.Append("select isnull(count(id_listado),0) as total from tb_listadomaterial where id_cliente = " & cliente & " And mes = " & mes & " And anio = " & anio & " And tipo = " & tipo & "")
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
            sql += "{listados: '" & dt.Rows(0)("total") & "'}"
        End If
        Return sql
    End Function
