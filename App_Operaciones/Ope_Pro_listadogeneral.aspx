    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#btgenera').click(function () {
                if (valida()) {
                    ...
                    if ($('input[name=radio]:checked', '#divmodal').val() == 2) {
                        if (mes == 1) {
                            mesa = 12
                            anioa = parseInt(anio) - 1 
                        } else {
                            mesa = parseInt(mesa) - 1
                        }
                    }
                    var xmlgraba = '<listado cliente= "' + $('#dlcliente').val() + '" mes= "' + $('#dlmes').val() + '" mesa = "' + mesa + '" anio= "' + anio + '"';
                    xmlgraba += ' anioa = "' + anioa + '" usuario= "' + $('#idusuario').val() + '" opcion = "' + $('input[name=radio]:checked', '#divmodal').val() + '" tipo="' + $('#dltipo').val() + '" />';
                    //alert(xmlgraba);
                ...
                if (valida1()) {
                    //if ($('#dltipo').val() == 1) {
                        if ($('#tblista tbody tr').length == 0) {
                            PageMethods.validalistados($('#dlcliente').val(), $('#dltipo').val(), $('#dlmes').val(), $('#txanio').val(), function (detalle) {
                                var datos = eval('(' + detalle + ')');
                                if (datos.listados > 0) {
                                    //alert('Los listados para el período seleccionado ya estan creados, no puede duplucar');
                                    cargatodo();
                                }
                                else {
            ...
            $('#btbuscap').click(function () {
                PageMethods.productol($('#txbusca').val(), $('#idinmueble').val(), $('#dlcliente').val(), $('#dltipo').val(), function (res) {
        ...
        function cargaptto() {
            PageMethods.presupuesto($('#dlcliente').val(), $('#dltipo').val(), $('#dlmes').val(), $('#txanio').val(), function (detalle) {
                var datos = eval('(' + detalle + ')');
                var ptto = parseFloat(datos.ptto);
                var usado = parseFloat(datos.usado);
                $('#txptto').val(ptto.toFixed(2));
                $('#txpttou').val(usado.toFixed(2));
        ...
        function cuentalistados() {
            PageMethods.contarlistados($('#dlcliente').val(), $('#dlmes').val(), $('#txanio').val(), $('#dlestado').val(), $('#dltipo').val(), function (cont) {
                ...
                <div class="content">
                    <div class="row" id="dvdatos">
                        <div class="box box-info">
                            ...
                            <div class="row"    >
                                <div class="col-lg-3">
                                    <input type="button" id="btcrear" class="btn btn-primary" value="Generar Listados masivos"/>
                                </div>
                                <div class="col-lg-1">
