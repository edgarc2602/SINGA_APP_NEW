...
Partial Class App_Mantenimiento_OP_Pro_VisitasSupervision
    ...
    Public Shared Function cliente() As String
        ...
        sqlbr.Append("where a.id_status =1 and b.id_lineanegocio= 2 order by nombre")
    ...
    Public Shared Function tecnico() As String
        ...
        sqlbr.Append("where id_status = 2 and id_puesto=20 ")
    ...
    Public Shared Function svPrograma(ByVal prm As String) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim cmd As New SqlCommand("sp_visitasupervision", myConnection)
    ...
    <Web.Services.WebMethod()>
    Public Shared Function buscaprograma(ByVal cliente As String, ByVal coordina As Integer) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""

        sqlbr.Append("select id_visitasupervision from tb_visitasupervision where id_cliente =" & cliente & " ")
        sqlbr.Append(" and id_coordinador = " & coordina & " and id_status =1")

        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
            sql += "{id:'" & dt.Rows(0)("id_visitasupervision") & "'}" & vbCrLf
        End If
        Return sql
    End Function
    ...
    Public Shared Function gtEstructura(ByVal prm As String) As String
        ...
        If xdoc.DocumentElement().Attributes("prg").Value <> 0 Then
            ...
            'sql += "select a.id_visitasupervision, a.id_cliente, a.id_coordinador, b.descripcion" & vbCrLf
            sql += "select a.id_visitasupervision, a.id_cliente, a.id_coordinador " & vbCrLf
            sql += "from tb_visitasupervision a" & vbCrLf
            'sql += "inner join tb_tipomantenimiento b on a.id_servicio=b.id_servicio" & vbCrLf
            sql += "where a.id_visitasupervision = " & xdoc.DocumentElement().Attributes("prg").Value & " and a.id_status = 1;"
            ...
            If dt.Rows.Count > 0 Then
                ...
                'sql += ", per: " & dt.Rows(0)("id_visitasupervision") & ", sup: " & dt.Rows(0)("id_coordinador") & ", tipo: '" & dt.Rows(0)("descripcion") & "'}"
                sql = "{cmd: true, pro: " & dt.Rows(0)("id_cliente") & " "
                sql += ", per: " & dt.Rows(0)("id_visitasupervision") & ", sup: " & dt.Rows(0)("id_coordinador") & "}"
            Else
                sql = "{cmd: false }"
            End If
        Else
            ...
            sql = "declare @dta xml;" & vbCrLf
            sql += "Set @dta = '" & prm & "';" & vbCrLf & vbCrLf
            sql += "select a.id_visitasupervision" & vbCrLf
            sql += "from tb_visitasupervision a" & vbCrLf
            sql += "inner join @dta.nodes('Param') t(c) on a.id_supervisor = c.value('@sup', 'int')" & vbCrLf
            sql += "and a.id_cliente = c.value('@pro', 'int') " & vbCrLf
            'sql += "and a.id_periodo = c.value('@per', 'smallint') " & vbCrLf
            sql += "and a.id_status = 0;"
            dt = New DataTable()
            da = New SqlDataAdapter(sql, myConnection)
            da.Fill(dt)
            If dt.Rows.Count > 0 Then
                sql = "{cmd: true, prg: " & dt.Rows(0)("id_visitasupervision") & "}"
            Else
                sql = "{cmd: false}"
            End If
        End If
    ...
    Public Shared Function gtPreventivo(ByVal prm As String) As String
        ...
        sql += "declare @dta xml;" & vbCrLf
        sql += "declare @fec date, @pgn int, @pla int, @sup int, @pro int;" & vbCrLf

        sql += "set @dta = '" & prm & "';" & vbCrLf & vbCrLf

        sql += "select @fec = convert(date, c.value('@fec', 'char(10)'), 103), @pgn = c.value('@pgn', 'int') "
        'sql += ", @pro = c.value('@pro', 'int') from @dta.nodes('Param') t(c);" & vbCrLf
        sql += " from @dta.nodes('Param') t(c);" & vbCrLf

        sql += "select @sup = a.id_coordinador, @pro = a.id_Cliente " & vbCrLf
        sql += "From tb_visitasupervision a " & vbCrLf
        sql += "inner join @dta.nodes('Param') t(c) on  a.id_visitasupervision = c.value('@prg', 'int');" & vbCrLf
        ...
        'sql += "	and a.Id_Plaza = " & xdoc.DocumentElement().Attributes("pla").Value & vbCrLf
        'sql += "    and a.Id_Supervisor = " & xdoc.DocumentElement().Attributes("sup").Value & vbCrLf
        sql += ") c" & vbCrLf
        sql += "where pgn = @pgn;" & vbCrLf & vbCrLf

        sql += "select b.id, b.inm, b.ordo as ren, c.ordo as col, count(a.id_ordensupervision) as num, max(a.id_ordensupervision) as numot" & vbCrLf
        sql += "from tb_ordensupervision a" & vbCrLf
        ...
        'sql += "and a.Id_Plaza = " & xdoc.DocumentElement().Attributes("pla").Value & vbCrLf
        'sql += "and a.Id_Empresa = " & xdoc.DocumentElement().Attributes("emp").Value & vbCrLf
    ...
    Public Shared Function svOrdenes(ByVal prm As String) As String
        ...
        Dim cmd As New SqlCommand("[sp_generasupervision]", myConnection)
        cmd.CommandType = CommandType.StoredProcedure
        cmd.Parameters.AddWithValue("@dta", prm)
        myConnection.Open()
        cmd.ExecuteNonQuery()
    ...
    Public Shared Function muestrapreventivos(ByVal pagina As Integer, ByVal campo As String, ByVal dato As String) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        'sqlbr.Append("select id_visitasupervision as 'td','', nombre as 'td','', descripcion as 'td','', coordinador as 'td','' ")
        sqlbr.Append("select id_visitasupervision as 'td','', nombre as 'td','', coordinador as 'td','' ")
        sqlbr.Append("from (  ")
        'sqlbr.Append("select ROW_NUMBER() over (order by id_visitasupervision) as rownum,  id_visitasupervision, b.nombre, '' descripcion, d.nombre + ' ' + d.paterno + ' ' + rtrim(d.materno) as coordinador ")
        sqlbr.Append("select ROW_NUMBER() over (order by id_visitasupervision) as rownum,  id_visitasupervision, b.nombre, d.nombre + ' ' + d.paterno + ' ' + rtrim(d.materno) as coordinador ")
        sqlbr.Append("  from tb_visitasupervision a  inner join tb_cliente b on a.id_cliente=b.id_cliente ")
        ...
        sqlbr.Append("inner join tb_empleado d on a.id_coordinador= d.id_empleado  ")

        If campo <> "0" Then
            sqlbr.Append("where " & campo & " like '%" & dato & "%'" & vbCrLf)
        End If
        sqlbr.Append(") as result" & vbCrLf)
        sqlbr.Append(" where RowNum BETWEEN (" & pagina & "  - 1) * 20 And " & pagina & " * 20 order by id_visitasupervision for xml path('tr'), root('tbody')")
    ...
    Public Shared Function contarpreventivos(ByVal campo As String, ByVal dato As String) As String
        ...
        sqlbr.Append("select COUNT(*)/20 + 1 as Filas, COUNT(*) % 20 as Residuos from tb_visitasupervision a inner join tb_cliente b on a.id_cliente=b.id_cliente " & vbCrLf)
    ...
    Public Shared Function consultapreventivo(ByVal prm As Integer) As String
        ...
        If prm <> 0 Then
            'sql = "declare @dta xml;" & vbCrLf
            'sql += "Set @dta = '" & prm & "';" & vbCrLf & vbCrLf
            'sql += "select a.id_visitasupervision, a.id_cliente, a.id_servicio, a.id_servicio, a.id_coordinador, b.descripcion" & vbCrLf
            'sql += "from tb_visitasupervision a" & vbCrLf
            'sql += "inner join tb_tipomantenimiento b on a.id_servicio=b.id_servicio" & vbCrLf
            'sql += "where a.id_visitasupervision = " & prm & " and a.id_status = 1;"
            sql = "declare @dta xml;" & vbCrLf
            sql += "Set @dta = '" & prm & "';" & vbCrLf & vbCrLf
            sql += "select a.id_visitasupervision, a.id_cliente, a.id_coordinador " & vbCrLf
            sql += "from tb_visitasupervision a " & vbCrLf
    ...
    Public Shared Function gtPrograma(ByVal prm As String) As String
        ...
        'sql += "Select *" & vbCrLf
        'sql += "from (" & vbCrLf
        'sql += "	Select a.id_visitasupervision As prg, rtrim(b.Nombre) As pro, rtrim(d.Descripcion) As ser" & vbCrLf
        ...
        'sql += "	inner join tb_cliente b on a.id_cliente = b.id_cliente" & vbCrLf
        'sql += "	inner join tb_tipomantenimiento d On a.id_servicio = d.id_servicio" & vbCrLf
        'sql += "	left outer join tb_ordentrabajo e On a.id_visitasupervision = e.id_programa " & vbCrLf
        'If xdoc.DocumentElement.Attributes("pro").Value <> 0 Then
        '    sql += "And a.id_cliente = " & xdoc.DocumentElement.Attributes("pro").Value & vbCrLf
        'End If
        'If xdoc.DocumentElement.Attributes("sup").Value <> 0 Then
        ...
        'End If
        'sql += "	and a.id_status = 0" & vbCrLf
        'sql += "	group by a.id_visitasupervision, b.Nombre, d.Descripcion, Case a.id_status When 0 Then 'Activo' else 'Cancelado' end" & vbCrLf
        'sql += ") a" & vbCrLf
        'sql += "where pgn = @pgn" & vbCrLf
        'sql += "order by pro;"
        ...
        sql += "from (" & vbCrLf
        'sql += "	Select (row_number() over(order by b.nombre, d.descripcion) - 1) / 200 As pgn" & vbCrLf
        sql += "	Select (row_number() over(order by b.nombre) - 1) / 200 As pgn" & vbCrLf
        sql += "	from tb_visitasupervision a" & vbCrLf
        sql += "	inner join tb_cliente b On a.id_cliente = b.id_cliente" & vbCrLf
        'sql += "	inner join tb_tipomantenimiento d on a.id_servicio = d.id_servicio" & vbCrLf
        ...
        sql += "from (" & vbCrLf
        'sql += "	Select a.id_visitasupervision As prg, rtrim(b.Nombre) As pro, rtrim(d.Descripcion) As ser" & vbCrLf
        sql += "	Select a.id_visitasupervision As prg, rtrim(b.Nombre) As pro " & vbCrLf
        sql += "	, isnull(DATENAME(M, max(e.fregistro)) + ' ' + datename(yy, max(e.fregistro)), '') As sup" & vbCrLf
        sql += "	, Case a.id_status When 0 Then 'Activo' else 'Cancelado' end as stt" & vbCrLf
        'sql += "	, (row_number() over(order by b.nombre, d.descripcion) - 1) / 200 as pgn" & vbCrLf
        sql += "	, (row_number() over(order by b.nombre) - 1) / 200 as pgn" & vbCrLf
        sql += "	from tb_visitasupervision a" & vbCrLf
        sql += "	inner join tb_cliente b on a.id_cliente = b.id_cliente" & vbCrLf
        'sql += "	inner join tb_tipomantenimiento d On a.id_servicio = d.id_servicio" & vbCrLf
        sql += "	left outer join tb_ordensupervision e On a.id_visitasupervision = e.id_ordensupervision " & vbCrLf
        If xdoc.DocumentElement.Attributes("pro").Value <> 0 Then
            sql += "And a.id_cliente = " & xdoc.DocumentElement.Attributes("pro").Value & vbCrLf
        End If
        If xdoc.DocumentElement.Attributes("sup").Value <> 0 Then
        ...
        End If
        sql += "	and a.id_status = 0" & vbCrLf
        sql += "	group by a.id_visitasupervision, b.Nombre, Case a.id_status When 0 Then 'Activo' else 'Cancelado' end" & vbCrLf
        sql += ") a" & vbCrLf
        sql += "where pgn = @pgn" & vbCrLf
        sql += "order by pro;"
    ...
    Public Shared Function cancelaot(ByVal id As Integer) As String
        ...
        Dim sql As String = "update tb_ordensupervision set id_status=4 where id_ordensupervision=" & id
