...
Imports System.Xml
Partial Class App_Almacen_Alm_Pro_SolicitudmaterialMantto
    ...
    Public Shared Function almacen(ByVal tipo As Integer) As String
        ...
        sqlbr.Append("select id_almacen, rtrim(nombre) as nombre from tb_almacen where id_status = 1 and tipo =" & tipo & " order by nombre ")
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        sql = "["
    ...
    Public Shared Function producto(ByVal clave As String) As String
        ...
        sqlbr.Append("Where a.clave = '" & clave & "'")
    ...
    Public Shared Function solicitud(ByVal idsol As String) As String
        ...
        sqlbr.Append("select id_almacenent, id_almacensal, id_cliente, convert(varchar(12), falta, 103) as falta, id_tecnico, id_inmueble, observacion, id_tipo, id_clave_cm, fdevolucion," & vbCrLf)
        sqlbr.Append("case when id_status = 1 then 'Alta' end as status " & vbCrLf)
        sqlbr.Append("from tb_solicitudmaterialmantto where id_solicitud = " & idsol & "")
        Dim da As New SqlDataAdapter(sqlbr.ToString, myConnection)
        Dim dt As New DataTable
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
            sql += "{almacen1:'" & dt.Rows(0)("id_almacenent") & "', cliente:'" & dt.Rows(0)("id_cliente") & "', fecha:'" & dt.Rows(0)("falta") & "',"
            sql += "almacen:'" & dt.Rows(0)("id_almacensal") & "',observacion:'" & dt.Rows(0)("observacion") & "', inmueble:'" & dt.Rows(0)("id_inmueble") & "',"
            sql += "tipo:'" & dt.Rows(0)("id_tipo") & "',cm:'" & dt.Rows(0)("id_clave_cm") & "',fdevolucion:'" & dt.Rows(0)("fdevolucion") & "', "
            sql += "tecnico:'" & dt.Rows(0)("id_tecnico") & "', status:'" & dt.Rows(0)("status") & "'}"
    ...
    End Function
    <Web.Services.WebMethod()>
    Public Shared Function cargadetalle(ByVal idsol As Integer) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        ...
        sqlbr.Append("From tb_solicitudmaterialdmantto a inner join tb_producto c on a.clave = c.clave inner join tb_unidadmedida d on c.id_unidad = d.id_unidad" & vbCrLf)
