...
Imports System.Xml
Partial Class Ventas_App_Ven_Con_Cliente_ubicacion
    ...
    Public Shared Function inmueble(ByVal cliente As Integer, ByVal estado As Integer) As String
        ...
        sqlbr.Append("select nombre, latitud, longitud from tb_cliente_inmueble where id_cliente = " & cliente & " and id_status = 1 and not latitud is null" & vbCrLf)
        If estado <> 0 Then
            sqlbr.Append("and id_estado = " & estado & "")
        End If
        ...
        If dt.Rows.Count > 0 Then
            For x As Integer = 0 To dt.Rows.Count - 1
                If x > 0 Then sql += ","
                sql += "{nombre:'" & dt.Rows(x)("nombre") & "'," & vbCrLf
                sql += "latitud:'" & dt.Rows(x)("latitud") & "', longitud:'" & dt.Rows(x)("longitud") & "'}" & vbCrLf
            Next
        End If
        sql += "]"
        Return sql
