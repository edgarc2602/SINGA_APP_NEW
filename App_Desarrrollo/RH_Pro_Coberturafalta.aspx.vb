...
Partial Class App_RH_RH_Pro_Coberturafalta
    ...
    Public Shared Function empleado(ByVal cliente As Integer, ByVal inmueble As Integer, ByVal fecha As String, ByVal periodo As Integer, ByVal tipo As String, ByVal anio As Integer) As String
        ...
        sqlbr.Append("select id_inmueble as 'td','', inmueble as 'td','', id_empleado as 'td','', nombre as 'td','',"& vbCrLf)
        sqlbr.Append("convert(varchar(12),fecha,103) as 'td','', mov as 'td','',(select 'Buscar' as '@value', 'btbusca btn btn-info' as '@class', 'button' as '@type' for xml path('input'), root('td'),type),''," & vbCrLf)
        sqlbr.Append("(select 'disabled' as '@disabled', 'form-control' as '@class', 'text' as '@type', case when cubre is not null then cubre end  as '@value' for xml path('input'), root('td'),type),'',"& vbCrLf)
        sqlbr.Append("(select 'disabled' as '@disabled', 'form-control' as '@class', 'text' as '@type', case when cubre is not null then empleado  end  as '@value' for xml path('input'), root('td'),type),'', " & vbCrLf)
        sqlbr.Append("case when cubre is not null then"& vbCrLf)
        ...
        sqlbr.Append("case when cubre is not null then '1' else '0' end as 'td','', case when cubre is not null then tipoemp else '0' end as 'td', '', posicion as 'td' from (" & vbCrLf)
        sqlbr.Append("select b.id_inmueble, c.nombre as inmueble, 0 as id_empleado,'POSICION VACANTE' as nombre, '" & vfecha & "' as fecha,'VACANTE' as mov, isnull(d.cubre,0) cubre," & vbCrLf)
        sqlbr.Append("isnull(e.paterno + ' ' + rtrim(e.materno) + ' ' + e.nombre,'') empleado," & vbCrLf)
        sqlbr.Append("case when d.cubre is not null then '1' else '0' end as cubierto, case when d.cubre is not null then d.tipoemp else '0' end as tipoemp, a.posicion" & vbCrLf)
        ...
        sqlbr.Append("left outer join tb_empleado_cubre d on  d.id_periodo = " & periodo & " and  d.tipo = '" & tipo & "' and d.anio = " & anio & " and a.posicion = d.posicion and c.id_inmueble = d.id_inmueble and  d.fecha = '" & Format(vfecha, "yyyyMMdd") & "'" & vbCrLf)
        sqlbr.Append("left outer join tb_empleado e on d.cubre = e.id_empleado" & vbCrLf)
        sqlbr.Append("where a.id_empleado =0" & vbCrLf)
        sqlbr.Append("union all " & vbCrLf)
        sqlbr.Append("Select a.id_inmueble, b.nombre As inmueble, a.id_empleado, c.paterno + ' ' +  rtrim(c.materno) + ' ' + c.nombre as nombre," & vbCrLf)
        sqlbr.Append("convert(varchar(12),a.fecha,103) as fecha, a.movimiento as mov,isnull(d.cubre,0) cubre, isnull(e.paterno + ' ' + rtrim(e.materno) + ' ' + e.nombre,'') empleado," & vbCrLf)
        sqlbr.Append("case when d.cubre is not null then '1' else '0' end as cubierto, case when d.cubre is not null then d.tipoemp else '0' end as tipoemp, f.posicion" & vbCrLf)
        ...
        sqlbr.Append("left outer join tb_empleado e on d.cubre = e.id_empleado" & vbCrLf)
        sqlbr.Append("left outer join tb_cliente_plantillap f on c.id_empleado = f.id_empleado" & vbCrLf)
        ...
        sqlbr.Append("and a.fecha = '" & Format(vfecha, "yyyyMMdd") & "') as result order by posicion, nombre for xml path('tr'), root('tbody')")
