...
Partial Class App_Finanzas_Fin_Con_Solicitudrecurso
    ...
    <Web.Services.WebMethod()>
    Public Shared Function contarsolicitud(ByVal fini As String, ByVal ffin As String, ByVal folio As Integer, ByVal cliente As Integer, ByVal empleado As Integer, ByVal estatus As Integer) As String
        Dim sqlbr As New StringBuilder
        Dim sql As String = ""
        ...
        sqlbr.Append("SELECT COUNT(*)/50 + 1 as Filas, COUNT(*) % 50 as Residuos FROM tb_solicitudrecurso" & vbCrLf)
        sqlbr.Append("where id_status = " & estatus & "" & vbCrLf)
    ...
    <Web.Services.WebMethod()>
    Public Shared Function solicitudes(ByVal fini As String, ByVal ffin As String, ByVal folio As Integer, ByVal cliente As Integer, ByVal pagina As Integer, ByVal empleado As Integer, ByVal estatus As Integer) As String
        Dim myConnection As New SqlConnection((New Conexion).StrConexion)
        Dim sqlbr As New StringBuilder
        ...
        sqlbr.Append("select id_solicitud as 'td','', tipo as 'td','', empleado as 'td','', empresa as 'td','', gasto as 'td','', " & vbCrLf)
        sqlbr.Append("fecha as 'td','', estatus as 'td','', total as 'td','',isnull(banco,'') as 'td','', isnull(clabe,'') as 'td','', isnull(cuenta,'') as 'td','', concepto as 'td',''," & vbCrLf)
        ...
        sqlbr.Append("")
        sqlbr.Append("from ( select ROW_NUMBER() over (order by a.id_solicitud) as rownum , id_solicitud, case when a.id_tipo = 5 then d.razonsocial else b.nombre + ' ' + b.paterno + ' ' + trim(b.materno) end as empleado, c.nombre as empresa," & vbCrLf)
        sqlbr.Append("convert(varchar(12),a.falta,103) as fecha, cast(a.total as numeric(8,2)) as total, f.descripcion as estatus," & vbCrLf)
        ...
        sqlbr.Append("case when a.id_empleado != 0 then h.descripcion else i.descripcion end as banco," & vbCrLf)
        sqlbr.Append("case when a.id_empleado != 0 then b.clabe else d.clabe end as clabe," & vbCrLf)
        sqlbr.Append("case when a.id_empleado != 0 then b.cuenta else d.cuenta end as cuenta," & vbCrLf)
        sqlbr.Append("a.concepto" & vbCrLf)
